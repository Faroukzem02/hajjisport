...
<head>
    ...
    <title>hajji sport - list de produits</title>
    <style>
        <?php
        include "style.css"
        ?>
...
<body>
    ...
    <div class="list">
        ...
        <h1><span><?= strtoupper($pid) ?></span></h1>
        <div class="products">
            <?php
            if (isset($_GET["pid"])) {
                $select_prods = $conn->prepare("SELECT * from `products` where category_fr='$pid'");
                $select_prods->execute();
                if ($select_prods->rowCount() > 0) {
                    while ($fetch_prods = $select_prods->fetch(PDO::FETCH_ASSOC)) {
            ?>
                        <a href="product?pid=<?= $fetch_prods["id"] ?>#product-container" class="product">
                            <div class="img">
                                <img src="imgs/prods/<?= $fetch_prods["img"] ?>" alt="">
                            </div>
                            <div class="name_price">
                                <p class="name"><?= $fetch_prods["nom"] ?></p>
                                <p class="price"><?= $fetch_prods["prix"] ?> DH</p>
                            </div>
