...
<body>
    ...
    <div class="list">
        ...
        <h1>Imprimer votre propre design</h1>
        <h4>sur le produit que vous aimez</h4>
        <div class="products">
            <?php
            $select_prods = $conn->prepare("SELECT * from `products` where category_fr ='homme' or category_fr='femme' or category_fr='enfant'");
            $select_prods->execute();
            if ($select_prods->rowCount() > 0) {
                while ($fetch_prods = $select_prods->fetch(PDO::FETCH_ASSOC)) {
            ?>
                    <a href="perso_form?pid=<?= $fetch_prods["id"] ?>" class="product">
                        <div class="img">
                            <img src="imgs/prods/<?= $fetch_prods["img"] ?>" alt="">
                        </div>
                        <div class="name_price">
                            <p class="name"><?= $fetch_prods["nom"] ?></p>
                            <p class="price"><?php echo $fetch_prods["prix"] + 200 ?> DH</p>
                        </div>
                        <p class="type"><?= $fetch_prods["type_fr"] ?></p>
                    </a>
            <?php
                }
            }
