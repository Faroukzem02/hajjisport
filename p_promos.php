...
if (isset($_POST["add_to_cart"])) {
    ...
    $select_cmds = $conn->prepare("SELECT * FROM `commands` WHERE user_id='$user_id' AND prod_id='$prod_id' AND statut='in progress' AND source='$source'");
    $select_cmds->execute();
...
<body>
    ...
    if (isset($_GET["pid"])) {
        $pid = $_GET["pid"];
        $select_prods = $conn->prepare("SELECT * from `promos` where id='$pid'");
                ...
                <div class="product-container" id="product-container">
                    ...
                    <div class="right">
                        ...
                        <div class="name_price">
                            ...
                            <div class="price">
                                <h2 style="color: rgb(240,7,7);text-decoration:line-through"><?= $fetch_prods["oldprix"] ?> DH</h2>
                                <h2><?= $fetch_prods["newprix"] ?> DH</h2>
                            </div>
                        </div>
                        <div class="buttons">
                        ...
                        <div class="shop">
                            ...
                            ?>
                                <form action="" method="post">
                                    <input type="hidden" name="prod_id" value="<?= $fetch_prods["id"] ?>">
                                    <input type="hidden" name="prod_price" value="<?= $fetch_prods["newprix"] ?>">
                                    <input type="hidden" name="source" value="<?= $fetch_prods["source"] ?>">
                                    <input type="text" name="fullname" placeholder="nom complet ..." maxlength="20" required value="<?= $fetch_users["fullname"] ?>">
                                    <input type="text" name="tel" placeholder="téléphone ..." required maxlength="15" value="<?= $fetch_users["tel"] ?>">
                                    <input type="text" name="adresse" placeholder="Adresse ..." required maxlength="40" value="<?php echo $fetch_users["adress"] . " , " . $fetch_users["ville"] . " , " . $fetch_users["pays"] ?>">
                                    <input type="number" name="qty" value="1" placeholder="Quantité" min="1" max="5" required>
                                    <?php
